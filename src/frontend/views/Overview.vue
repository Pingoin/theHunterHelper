...
<div class="home col-xl-12">
    ...
    <table class="table table-striped table-hover table-header-rotated">
        <thead>
            <tr>
                <th scope="col" class="calibers">
                    Kaliber<span class="arrow"></span>
                </th>
        ...
        <tbody>
            <template v-for="caliber in calibers">
                ...
                <tr v-for="ammo in caliber.ammos" :key="caliber.caliberID + ammo.ammoType">
                    <td>{{ ammo.ammoType }}</td>
                    <td scope="col" v-for="animal in animals" :key="animal.animalID" :class="
                ammo.minClass <= animal.animalClass &&
                ammo.maxClass >= animal.animalClass
                  ? 'true'
                  : 'false'
              ">
                        {{ ammo.minClass }}-{{ ammo.maxClass }}/{{ animal.animalClass }}
                    </td>
                </tr>
            </template>
        </tbody>
